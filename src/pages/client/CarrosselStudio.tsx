...
import {
  ...
  LayoutTemplate, Image as ImageIcon, Layers
} from 'lucide-react';
...
import {
  buscarPautas, gerarAngulos, gerarCarrossel,
...
export default function CarrosselStudio() {
  ...
  function handleDownloadHTML() {
    ...
    const { legenda, hashtags } = resultadoCompleto.copy;
    const txt = `LEGENDA:\n${legenda}\n\nHASHTAGS:\n${hashtags.join(' ')}`;
    const blob = new Blob([txt], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `legenda-carrossel-${Date.now()}.txt`;
    a.click();
      ...
      <div className="max-w-3xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">Crie carrosseis profissionais com IA em minutos</p>
          </div>
        </div>
          ...
          <motion.div key={etapa} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.2 }}>
              ...
              <Card>
                <CardContent className="p-6 space-y-6">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mb-4">Digite um tema ou deixe em branco para a IA escolher automaticamente.</p>
                    <textarea value={tema} onChange={(e) => setTema(e.target.value)} placeholder="Ex: alimentação saudável, marketing digital, dicas de finanças..." className="w-full min-h-[100px] rounded-lg border border-input bg-background px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ring" />
                  </div>
                  <div>
            ...
            {etapa === 1 && (
              <div className="space-y-3">
                ...
                <p className="text-sm text-muted-foreground">Pedro Pauta encontrou 5 ideias. Clique em uma para continuar.</p>
                {pautas.map((pauta) => (
                ...
                <Button variant="outline" className="w-full" onClick={() => setEtapa(0)}>
                  <ArrowLeft className="w-4 h-4 mr-2" /> Voltar e mudar tema
                </Button>
              </div>
            )}
            ...
            {etapa === 2 && (
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Clara Carrossel gerou 5 abordagens para: <strong>{pautaSelecionada?.titulo}</strong></p>
                </div>
                {angulos.map((angulo) => (
                    ...
                    <CardContent className="p-4 flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <Badge variant="secondary" className="text-xs">{angulo.tipo}</Badge>
                        </div>
                ...
                <Button variant="outline" className="w-full" onClick={() => setEtapa(1)}>
                  <ArrowLeft className="w-4 h-4 mr-2" /> Voltar às pautas
                </Button>
              </div>
            )}
            ...
            {etapa === 3 && (
              <div className="space-y-4">
                ...
                {resultadoCompleto?.copy.legenda && (
                  <Card>
                    <CardContent className="p-4 space-y-2">
                      <p className="text-xs font-semibold text-muted-foreground">📝 LEGENDA PARA O INSTAGRAM</p>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Button className="flex-1 gap-2 border-emerald-500 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500/20 hover:text-emerald-400" variant="outline" onClick={() => setShowStudio(true)}>
