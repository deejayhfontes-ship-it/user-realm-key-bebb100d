...
import {
    ArrowLeft, Download, Eye, Loader2, FolderOpen,
    Image as ImageIcon, Film, FileText, FileType, Grid3X3, Search, Package
} from 'lucide-react';
...
export default function ClientMateriaisDetail() {
    ...
    if (!campanha) {
        return (
            <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center text-white">
                ...
                <h2 className="text-xl font-semibold text-white/50">Material não encontrado</h2>
                <button onClick={() => navigate(-1)} className="mt-4 text-emerald-400 hover:underline text-sm">
                    Voltar
                </button>
            </div>
        );
            ...
            <main className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div className="relative overflow-hidden rounded-2xl border border-white/5 bg-[#0a0a0a] mb-8">
                    {campanha.cover_image ? (
                        <div className="aspect-[21/9] w-full">
                            <img src={campanha.cover_image} alt={campanha.title} className="w-full h-full object-cover" />
                            <div className="absolute inset-0 bg-gradient-to-t from-[#050505] via-transparent to-transparent" />
                        </div>
                    ) : (
                        <div className="aspect-[21/9] w-full bg-gradient-to-br from-emerald-500/20 to-teal-500/10 flex items-center justify-center">
                            <FolderOpen className="w-20 h-20 text-white/10" />
                        </div>
                    )}
                ...
                {!campanha.drive_folder_id ? (
                    <div className="rounded-2xl border border-white/5 bg-[#0a0a0a] p-12 text-center">
                        ...
                        <p className="text-white/30 text-sm">O administrador precisa vincular uma pasta do Google Drive.</p>
                    </div>
                ) : loadingFiles ? (
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 mb-6">
                            <div className="relative flex-1">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-white/30 focus:outline-none focus:border-emerald-500/50 transition-all text-sm"
                                />
                            </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                                {filteredFiles.map((file) => {
                                    ...
                                    return (
                                        <div
                                            key={file.id}
                                            className="group rounded-xl border border-white/5 bg-[#0a0a0a] overflow-hidden hover:border-emerald-500/20 transition-all"
                                        >
                                            {/* Thumbnail */}
                                                ...
                                                <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                                                    ...
                                                    {file.webContentLink && (
                                                        <a
                                                            ...
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="p-2.5 rounded-xl bg-emerald-500/80 hover:bg-emerald-500 transition-all"
                                                            title="Baixar"
                                                        >
