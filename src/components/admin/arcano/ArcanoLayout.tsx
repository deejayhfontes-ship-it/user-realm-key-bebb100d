...
export function ArcanoLayout({ children }: ArcanoLayoutProps) {
    ...
    return (
        <div className="flex h-screen w-full overflow-hidden" style={{ background: '#0d0714' }}>
            {/* Sidebar */}
            <aside className="w-[260px] shrink-0 flex flex-col border-r border-white/5 overflow-y-auto"
                ...
                <div className="px-4 py-4 border-b border-white/5">
                    ...
                    >
                        ...
                        <span className="font-bold text-sm text-white">arcanoapp</span>
                    </button>
                </div>
                    ...
                    <div className="pt-3 pb-1">
                        ...
                        {sidebarTools.map((tool) => (
                            <NavLink
                                ...
                                to={tool.href}
                                className={({ isActive }) => cn(
                                    'flex items-center justify-between gap-2.5 px-3 py-2 rounded-lg text-sm transition-all',
                                    isActive
                            ...
                            >
                                ...
                                {tool.badge && (
                                    <span
                                        ...
                                        style={{ background: tool.badgeColor || '#7c3aed' }}
                                    >
                                        {tool.badge}
                                    </span>
                                )}
            ...
            {/* Conteúdo principal */}
            <main className="flex-1 overflow-y-auto" style={{ background: '#0d0714' }}>
                {children}
            </main>
        </div>
    );
