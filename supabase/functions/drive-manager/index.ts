...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const { action, protocol_code, display_name, type, customer_email, pedido_id, parent_folder_id } =
            await req.json()
        ...
        if (action === 'CREATE_FOLDER') {
            ...
            // Create folder in Google Drive (uses parent_folder_id if provided, else ROOT)
            const token = await getAccessToken(sa)
            const targetParent = parent_folder_id || ROOT_FOLDER_ID
            const folder = await createDriveFolder(token, folderName, targetParent)
            const folderUrl = `https://drive.google.com/drive/folders/${folder.id}`

            console.log(`✅ Pasta criada: ${folderName} (${folder.id}) em ${targetParent}`)
